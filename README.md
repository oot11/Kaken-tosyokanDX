...
1.本棚の画像の撮影時
書籍特定において、撮影した画像をすべて保存し、後から物体検出・OCRを行う方式では、SDカードに膨大なデータが蓄積されてしまう
という問題がある。しかしリアルタイム検出だと、処理速度が低下する というデメリットがある

2. 物体検出モデルの精度改善
   文字が薄くなっているものや本の幅が狭いものなどで誤検出・未検出が起きる

3. 検出結果とデータベースの連携
   データベースとの連携を作っておらず、本の背表紙情報を読み取るところまでしかできていない
